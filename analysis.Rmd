...
The approach to this analysis employs a supervised classification technique with a decision tree classifier---a super handy tool that learns to identify land cover categories based on examples we provide, or training data for short. This is a deliberate and informed process, where we train the model with specific , known data points, which in turn allows the algorithm to apply patterns to classify a larger, more ambiguous area.

The end goal is to create a clear and accurate land cover classification map of the study area. This map will serve as a foundation for monitoring environmental changes and guiding decisions in land management and policy. By ensuring each pixel in the Landsat imagery is correctly classified, we aspire to produce a map that is as informative as it is functional.
